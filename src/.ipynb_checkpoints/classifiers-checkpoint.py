...
def drop_zero_cols_mul(X_train: pd.DataFrame, X_dict: dict, X_any: pd.DataFrame):
    ...
    for i, X in X_dict.items():
        ...
        X_dict[i] = X
    X_any = X_any.drop(cols_to_drop, axis=1)
    return X_train, X_dict, X_any
...
def scale_features_mul(X_train: pd.DataFrame, X_dict: dict, X_any: pd.DataFrame):
    ...
    for i, X in enumerate(X_dict):
        X = pd.DataFrame((X.values - all_means) / all_stds, columns=X_train.columns)
        X_dict[i] = X
    X_any = pd.DataFrame((X_any.values - all_means) / all_stds, columns=X_train.columns)
    return X_train, X_dict, X_any
...
def fit_classifiers(X_train: pd.DataFrame, y_train: pd.DataFrame, models: list, cv=False):
    ...
    for model in models:
        ...
        if model=='logistic_regression':
            clf=train_LogisticRegresion(X_train, y_train, cv)
