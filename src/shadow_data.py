...
def generate_dataset_parallel(df_aux: pd.DataFrame, df_target: pd.DataFrame, meta_data: list, target_record: pd.DataFrame, df_eval: pd.DataFrame, in_dataset: bool,
                                    generator_name: str, n_synth: int, n_original: int, seeds_train: list, seeds_eval: list, idx: int, shadow_datasets: list, shadow_membership_labels: list,
                                    evaluation_datasets: list, evaluation_membership_labels: list, epsilon: float, train: bool):
    """
    ...
    if train: 
        ...
        synthetic_dataset = generator.fit_generate(dataset=df_train, metadata=meta_data,
                                                size=n_synth, seed=seeds_train[idx])
        enablePrint()
    else:
        if in_dataset:
        ...
        blockPrint()
        synthetic_dataset = generator.fit_generate(dataset=df_train, metadata=meta_data,
                                                size=n_synth, seed=seeds_eval[idx])
        enablePrint()
...
def generate_datasets_parallel(df_aux: pd.DataFrame, df_target: pd.DataFrame, meta_data: list, target_record: pd.DataFrame, df_eval: pd.DataFrame,
                                     generator_name: str, n_synth: int, n_original: int, n_datasets: int, seeds_train: list, seeds_eval: list, epsilon: float,
                                     train: bool):
    """
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for i in range(n_datasets*2):
            ...
            futures.append(
                executor.submit(
                    generate_dataset_parallel, df_aux=df_aux, df_target=df_target, meta_data=meta_data, target_record=target_record, df_eval=df_eval, in_dataset=in_dataset,
                    generator_name=generator_name, n_synth=n_synth, n_original=n_original, seeds_train=seeds_train, seeds_eval=seeds_eval, idx=idx,
                    shadow_datasets=shadow_datasets, shadow_membership_labels=shadow_membership_labels, evaluation_datasets=evaluation_datasets,
                    evaluation_membership_labels=evaluation_membership_labels, epsilon=epsilon, train=train
                )
            )
...
def generate_datasets(df_aux: pd.DataFrame, df_target: pd.DataFrame, meta_data: list, target_record_id: int, df_eval: pd.DataFrame, generator_name: str,
    ...
    return generate_datasets_parallel(df_aux=df_aux, df_target=df_target, meta_data=meta_data, target_record=target_record,
                                      df_eval=df_eval, generator_name=generator_name, n_synth=n_synth, n_original=n_original,
                                      n_datasets=n_datasets, seeds_train=seeds_train, seeds_eval=seeds_eval, epsilon=epsilon, train=False)