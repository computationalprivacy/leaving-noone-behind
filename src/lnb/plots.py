...
def plot_mia_scores(mia_results: list, output_path: str = None):
    ...
    sns.barplot(plot_df, x='record', y='accuracy', hue='Model', ax=axs[1], palette='tab10')
...
def mia_results_to_df(mia_results: list):
    ...
    for model in m[1].keys():
        df_temp = pd.concat([pd.DataFrame({ 
            'auc': mia_results[i][1][model]['auc'],
            'accuracy': mia_results[i][1][model]['accuracy'],
            'model': model
        },index=[mia_results[i][0]]) for i in range(len(mia_results))])
        df = pd.concat([df, df_temp])
    df = df.reset_index().rename({'index': 'record'}, axis=1)
    df = df.assign(
        Model = df.model.map(lambda x: model_names[x])
    )
    return df

model_names={'random_forest': 'Random Forest', 'logistic_regression': 'Logistic Regression', 'mlp': 'MLP'}